...
## CCM 影像調整流程
### 準備工作
- **拍攝設備**：架設好相機或鏡頭模組，確保在標準光源（如 D65、A光源）下進行。
...
![24colorchart](../image/Study_note_image/CCM_1/24color_Lab.png "24colorchart_Lab")
- **計算校正矩陣**：利用最小平方法（Least Squares Method）等演算法，計算出能將實際顏色轉換到目標值的 3 × 3 矩陣係數。
- **約束條件檢查**：套用矩陣時需確保每行係數和為 1（即 R+G+B = 1），以維持白色區域不跑色、不破壞白平衡。
- **微調飽和度**：根據主觀視覺或色差指標（如 Δ E），微調矩陣中的飽和度強弱，完成最終燒錄與驗證。
### 調整技巧
- 調整矩陣
![調整矩陣](../image/Study_note_image/CCM_1/CCM_1_1.PNG "調整矩陣")
    - 紅偏橘: 減小rg，或增大rb
    - 紅偏粉: 增大rg，或減小rb
    - 綠更嫩: 減小gb，同時減小gr
    - 膚色偏黃:增大gb
    ...
    - 黃偏綠/藍偏紫:減小br，或增大bg
    - 黃偏紅/藍偏青:增大br，或減小bg
![調整方向](../image/Study_note_image/isp_tuneing_process/ccm_5.png "調整方向")
## 關鍵特性 (Sam 的小筆記)
...
1. CIE 1976 ($ΔE^*_{ab}$​ )：最基礎的計算方式，採用 歐幾里得距離（Euclidean distance） 公式：  
## $$ΔE^*_{ab}​ = \sqrt{(L^*_2 −L^*_1​ )^2 +(a^*_2​ −a^*_1​ ) ^2 +(b^*_2​ −b^*_1 )^2}$$
...
## $$ΔC^∗ = \sqrt{(a^*_2​ − a^*_1​)^2 +(b^*_2 −b^*_1​)^2}$$
...
建議優化指標：專業軟體（如 Imatest）強烈建議使用 $ΔE_{2000}$  或 $ΔE_{94}$作為優化指標，因為它們比基礎的$ΔE_{ab}$ 更接近人眼的實際視覺感受。
誤差數值標準：  
...
典型相機表現：標準三色（RGB）感測器經過 CCM 校正後，最小平均 $ΔE_{2000}$通常落在 3 左右。  
### 四、 應用限制  
...
### 3. 內插邏輯（Interpolation）  
當偵測到的色溫介於兩組標定光源之間時，ISP 會根據權重對這兩組 CCM 矩陣的每一個係數進行線性內插，動態生成一組新的矩陣。  
數學原理： 假設偵測到的色溫為 T，它落在標定色溫 $T_low​$ 與 $T_high$​之間。  
## 計算權重 $w=\frac{T−T_{low}}{T_{high}​−T_{low}}$
## 最終矩陣 $M_{final}​=(1−w)×M_{low​}+w×M_{high}$
​
矩陣中的 9 個係數（$C_{00}$到 $C_{22}$​）均會依照此權重重新組合。  
### 4. 內插時的約束條件  
為了確保內插後的色彩過渡平滑且不產生異常，通常會遵守以下邏輯：
保持白平衡：確保內插後的矩陣每一行之總和依然為 1（即 $C_{i0}​+C_{i1}​+C_{i2}​=1$），這樣即使在內插過程中，白色區域依然能保持 R=G=B 的特性，不會發生偏色。
色溫分區策略：現代 ISP（如高通 QCOM、聯發科 MTK）會設定多個色溫區間點，並提供平滑過渡曲線，防止在光源切換時畫面色彩出現明顯跳變。  
### 5. 其他影響因素的綜合調整  
除了單純的色溫內插，部分 ISP 還會根據 **曝光增益（Gain**或 AI 場景識別結果來微調內插權重。例如在低照度（高增益）下，可能會動態縮小矩陣非對角線係數的權重，以抑制因色彩校正而放大的噪點。